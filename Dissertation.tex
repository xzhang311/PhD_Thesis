...
\end{tabular}} 
\caption{Network structure of proposed text to image approach}
\label{fig: txt2img} 
\end{figure}
...
Given a $k$ words sentence, I first map each word to a 400 dimension feature called Word2vec embeddings \cite{mikolov2013efficient} which better encodes latent semantic meaning of words. The generator network is denoted $G$: $\mathbb{R}^{400\times k} \times \mathbb{R}^{D\times k}$, the discriminator as $D$: $\mathbb{R}^{D\times k} \rightarrow \lbrace0, 1\rbrace^k$, $D$ is the dimension of the image. 
