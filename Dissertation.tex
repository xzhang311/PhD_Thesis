...
\end{tabular}}
\caption{A screen-shot of street view point cloud data used in my research. There are 9 million points that are contained in this scene.}
\label{fig: point cloud labeling}
\end{figure} 

\Section{The Proposed Solution} \label{sec: solution}
...
In this thesis, I address recognition problems of 2D \& 3D visual data using learning-based methods. There are two factors determining the results of a learning process. 
...
\Subsection{Creation of Synthetic Data in Data Space} Machine learning algorithms usually suffer from lacking enough training data. In existing methods data augmentation is used to increase the amount of training data. In data augmentation, new examples are generated as perturbed versions of the original data. For example, to augment images; the original images are stretched, added noise, or partially cropped. Images generated in this way form a distribution in parameter and feature space which is centered around the original images. Thus, these new images do not introduce many new pieces of information in this case. Therefore, the learner may fail to learn a more general decision boundary from the resulting data, and the increase in performance will be limited.
...
\Chapter{Data Synthesis in Data Space} \label{chapter: data space}
...
(a). The synthetic prototypes are then created to characterize primary structures of a roof represented by extracted edges. The way I create synthetic models is to simplify and convert roof edge images to a parametric model in which edges are connected by a few control points. By adjusting the position of the control points and drawing lines between the points, I can simulate different variations of roof edge images. All noises and line patterns introduced by actual roof edge images are ignored in synthetic images, which makes it easier to create synthetic images. However, without presenting these noises and patterns in synthetic images potentially increase the distance between actual data and synthetic data, thus lower the accuracy of a classifier trained using synthetic data. For all kinds of roof styles except flat I classified in my work, I intuitively design the parametric models of these roof edge images as the ones shown in Figure \ref{fig: controlpoints}.
...
\begin{tabular}{cc}
  \resizebox{0.4\textwidth}{!}{\rotatebox{0}{
  \includegraphics{Pic/PDF/Txt2Img/259_1.pdf}}}
  &
  \resizebox{0.4\textwidth}{!}{\rotatebox{0}{
  \includegraphics{Pic/PDF/Txt2Img/259_15.pdf}}}
  \\
  \begin{tabular}{c} This flower has one large purple \\
                     and white petal with one yellow \\
                     orange receptacle.
  \end{tabular}
  &
  \begin{tabular}{c} This flower has flat long and skinny \\
  					  yellow petals in one slightly upturned \\
  					  ring configuration.
  \end{tabular}
  \\
\end{tabular}}
...
This method can comfortably accommodate more new roof styles by re-training of the new dataset with the new roof styles added. Because I implement a bag of words model. 
...
Synthetic roofs are generated as smoothed and regular 3D meshes. Extracting features directly from this kind of synthetic roof will not work well when predicting on actual roof data. Because for real-world roof data, roof surface is bumpy and could contain different objects such as air conditioner, windows, and even holes. A solution used in my work is to compensate missing information in synthetic data when extracting features. Specifically, I am going to learn additional patterns on actual data and design features which can encode this information in the computation.

This was done from different aspects in \cite{ZX:14}. First, features in this work are designed to characterize spatial and context information of local point.
...
\noindent \textbf{i.\;Eigen Features (EF).} I compute the eigenvalues: $\lambda_1, \lambda_2, \lambda_3$ of the covariance matrix of neighbors $N_i$ centered at $p_i$. I then add following features for $p_i$: $\lambda_1$, $\lambda_2$, $\lambda_3$, $\lambda_3-\lambda_2$, $\lambda_2-\lambda_1$, $\lambda_1/(\lambda_1+\lambda_2+\lambda_3)$, $\lambda_2/(\lambda_1+\lambda_2+\lambda_3)$, $\lambda_3/(\lambda_1+\lambda_2+\lambda_3)$, yielding 8 features in total. These features capture the non-planarity of points around $p_i$.
...
\noindent\textbf{ii.\;Point Feature Histogram (PFH).} These are point cloud features based on the histogram which is described in \cite{RBR08}. Taking into account computational efficiency, three subdivisions of the features range are used in the feature histogram which yields $3^3=27$ features.
...
Synthetic data are created as locally perfectly planar or intersection of perfect flat surfaces. However, this usually is not a case in actual data where surfaces are locally irregularly bumpy and disrupted by outliers and noises. To better encode these irregularities in the spatial features, I learn this bumpiness from actual data and simulate them in synthetic data when computing above spatial feature. 

In my work bumpiness is defined as the distance a point moving up and down along vertical direction. To model the bumpiness in my work, I make following assumptions. First, bumpiness is correlated to a slope of a local surface where points on the flat surface are less likely to have the bumpy effect than points on the slope. Second, bumpiness follows a normal distribution in direction of point's normal for points on the similar slope surface, shown in Figure \ref{fig: BumpinessDistribution}. These assumptions though are quite heuristics; it makes sense to have them in this work. Because of laser signal decay much faster when bouncing back from slope surface thus the noise and outlier variance is larger for points on a surface with a larger slope.    
...
To this end, I have obtained the polynomial relationship between point slope and the variance of the distribution of point offset with which I can encode this randomness in features generated from synthetic data. Given a synthetic roof, I first compute the slope of every point on the roof, after which an adjustment of points location along a vertical axis is applied to each point using polynomial model just obtained. Such position adjustments have been applied to all features introduced previously. An example in Figure \ref{fig: Gaussian Smooth} shows a comparison of a spin image between using point adjustment and without.
...
The proposed approach has been tested on two datasets. In the first dataset, 3290 buildings were extracted from Chicago urban area. In the second dataset 3290 buildings were extracted from a San Francisco urban area. The two datasets have different characteristics and kinds of degradations.

In the first dataset, roof points are irregular, and the shape of the roof is decayed to some extent. This is because roofs in this dataset were originally produced from a highly down-sampled aerial LiDAR. The roof points in the second dataset have relatively low resolution and uneven distribution across the roof surface. Both datasets contain roof points of building only. The roofs were labeled to one of $9$ target roof styles according to their appearances. A roof is labeled as UNKNOWN when either the roof is not recognizable, or its roof style cannot be categorized into one of the eight styles. When a roof is composed of multiple styles, I label the roof according to the style of the largest component. The distribution of roof styles in the two datasets is shown in Figure \ref{fig: Roof Distribution}.
...
The confusion matrices of the proposed approach on the two datasets are shown in Figure \ref{fig: Confusion Matrix}. As can be observed, the proposed approach performs well in classifying most of the roof styles. Errors mostly come from the confusion between the hip and gable roof styles, which are similar. Indeed, visual examination confirms that in my dataset many hip roofs resemble gable roofs due to shape erosion.
...
As can be observed, the proposed approach achieves stable performance on most of the roof styles. For roof styles with a relatively low score, the curves present are ascending which could suggest that a better score can be obtained once more training data is included.
...
Exp. 1 (sup) & KITTI w. labels & N.A. & KITTI test set \\ \hline
Exp. 2 (sup) & Chairs w. labels & KITTI w. labels & KITTI test set \\ \hline
Exp. 3 (sup) & \begin{tabular}[c]{@{}l@{}}Chairs w. labels + \\ KITTI w. labels\end{tabular} & N.A. & KITTI test set \\ \hline
Exp. 4 (unsup) & KITTI raw w/o. labels & N.A. & KITTI test set \\ \hline
Exp. 5 (sup) & \begin{tabular}[c]{@{}l@{}}Chairs w. labels + \\ KITTI w. predicted labels\end{tabular} & N.A. & KITTI test set \\ \hline
Exp. 6 (sup) & \begin{tabular}[c]{@{}l@{}}Chairs w. labels +\\ KITTI w. predicted labels\end{tabular} & KITTI w. labels & KITTI test set \\ \hline
...
\Section{Learning features from synthetic data}
...
Various types of distance transform can be produced using different distance metrics $d(\cdot)$. In \cite{AR:66}, city-block distances are used, and a two-step linear algorithm is proposed to compute the distances. Euclidean distances approximations are provided by Borgefors \cite{GB:86}, Montanari \cite{UM:68}, and Danielsson \cite{PD:80}. An efficient squared Euclidean distance computation algorithm is described by Felzenszwalb \cite{PF:04}, Felzenszwalb's algorithm can be generalized to compute other distances. 

\Subsection{Extended Chamfer Matching} I extend the Chamfer matching in several ways. First, I extend the necessary Chamfer matching and obtain additional robustness by jointly minimizing the spatial distance between pixels and the misalignment of edge orientations. A similar distance metric is used in \cite{ML:10} where a linear representation of edges is generated to model the edge orientation. The proposed approach avoids the need for a linear representation of edges by computing the edge orientation directly from the images.
...
\Subsection{Edge Distance Variance} To have a robust matching result, the matching error of a pixel $v_i$ should not solely depend on $d(v_i^{\scalebox{.5}{\textit{U}}}, u_j)$. This is because it is possible that $v_i^{\scalebox{.5}{\textit{U}}}$ will be an incorrect edge pixel in the target image that happened to have a small error. Based on this consideration, I argue that to get a confident measurement of the matching error it is necessary to take into account the matching error of neighboring pixels.
...
Two kinds of metrics were used in my evaluation. In the first metric, I compute the average ratio of overlap area in the two datasets. In the second metric, I consider only results with area overlap above a specific rate ($85\%$ and $90\%$). 

In the first test where all the buildings were included, it could be observed in Figure \ref{fig: result comparison} that my algorithm generates better results compared with DCM and CM at almost every setting of $\lambda$. In both datasets, my algorithm maintains a similar and consistent performance, while DCM performs differently on the two datasets.
...
To test proposed algorithm's strength in finding matching targets under partial occlusion, I accurately test and evaluate the algorithms' performances on 53 buildings from San Francisco and 74 buildings from Chicago which are significantly occluded by shadows. I use a parameter of $\lambda=0.7$ in this test. The results are shown in Table \ref{table: building in shadow}. As can be observed, the proposed algorithm achieves 80\% accuracy on both datasets compared with about 50\% accuracy when using DCM and 30\% accuracy when using the original Chamfer matching. Some examples of matching incomplete buildings are shown in Figure \ref{fig: incomplete building results}. 

I finally tested the improvement obtained by the global constraint for $\lambda=0.7$.For both data sets, I obtain at least $2\%$ improvement regarding accuracy. The comparisons of alignment results with and without global constraint alignment are given in Table \ref{table: gc}.
...
synthetic data to generate the better feature representations
...
CNN) and encoded image features (in SVM). The second column shows the
...
Synthetic data help to learn a better classifier. I designed another group of experiments. In these experiments three different configurations of data are either reconstructed and encoded using the proposed MCAE, then used to train a CNN or an SVM in the experiments. All results from these experiments are compared in Table \ref{Tab: RoofBetterData} and Table \ref{Tab: DigitBetterData} respectively. An interesting thing to notice is that in experiments, using synthetic data can only achieve the same result as using a combination of real
...
\Chapter{Data Synthesis in Feature Space} \label{chapter: feature space}
...
Several existing approaches claim handling imbalanced learning better than SMOTE. Such claims are typically validated using empirical tests without a theoretical guarantee and in some instances may not extend to new datasets. In this section, I provide a theoretical guarantee showing that CGMOS is expected to work better than SMOTE in training process.

Let $D=\{(x_j, y_j)\}_{j=1}^n$ be a training dataset. Let $W(D)=\{W(x_i)\}_{i=1}^n$ be the sample weights computed using Eq. \ref{eqn: weight}. 
\newline
\newline
...
\Subsection{Datasets} 30 real-world datasets were randomly chosen from the UCI machine learning repository \cite{Lichman:2013} for empirical testing of CGMOS. Most of the datasets were released within the past ten years. As some of the datasets contain samples of more than two classes, I convert such datasets to a binary classification problem by keeping the class with the least data and merging all other classes. A summary of the test collections is provided in Table \ref{tab: realdata}.
...
\Subsection{Experiments on Artificial Datasets}
