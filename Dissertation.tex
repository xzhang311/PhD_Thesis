...
Data labeling is a tedious and time-consuming step of a machine learning process and may be very expensive especially if experts are involved. To tackle this problem and to reduce the cost spent on data labeling, several strategies and techniques had been proposed in last two decades.

A technique that creates synthesized data by over-sampling existing data has been extensively used in computer vision and machine learning. The most popular one among these techniques is SMOTE \cite{CN:02} which over-samples a minority class by using interpolation in feature space. Such interpolation gives a broader and more certain decision region to minority classes. Hence, the learner learns a less-biased boundary between the classes.

Instead of creating new synthetic data, a technique called semi-supervised learning will start from a small amount of labeled data and gradually grow and propagate to get more data labeled. To use unlabelled data, several assumptions have to be made to the underlying distribution of data, such as a smoothness assumption, a cluster assumption, and a manifold assumption\cite{CO:06}. Under this configuration, semi-supervised learning attempts to use this combination on labeled and unlabelled data were made. The idea is to improve the classification performance by propagating the label of labeled data to unlabelled data to expand the set of the data with labels.  

All of the learning schemas and techniques mentioned above have been successfully applied by researchers in the of computer vision and machine learning for decades. However, almost all of these methods are based on the assumption that the distribution of the data is known. Such assumption usually fails unless either data set with a distribution that covers every possibility in feature space is given, which pushes us back to the problem of data acquisition, or the data of training set and testing set are precisely drawn from the same distribution. An example that can explain this phenomenon is given by the problem of recognizing the human body motion. To have a high accuracy recognition of human body motion, a human body model with a high order of degrees of freedom is assumed with each degree of freedom representing a dimension in movement space. If the body movements in training set and testing set are different, (say training set containing arm movement only but testing set containing head tuning only), then the learned model has little or even no chance to recognize the testing set. 

In many recognition problems, humans have prior knowledge regarding the structure and the parameter domain of the problem. Take the problem of roof style recognition from satellite images, for example, the goal in this problem is to recognize the roof style of a given roof satellite image. The target labels are hip, halfhip, gable, pyramid, gambrel, and flat. The recognition of the roof style usually is conducted by recognizing features such as ridgelines and valley lines on the roof. However, due to the degradation of image quality and occlusion caused by shadows, it is hard to detect all of these features, the cases of which are shown in Figure \ref{fig: roof edge demonstration}. To solve this problem, since we know how the roof is built and we know where those ridge lines and valley lines are located on the roof, hundreds of thousands of synthetic roofs containing these features could be created and then used to train a recognizer. 
...
In this researches, I address recognition problems of 2D \& 3D visual data using learning-based methods. There are two factors determining the result of a learning process. 
...
The generator $G$ implicitly defines a probability distribution $p_{g}$ as the distribution of the samples $G(z)$ obtained when $z\sim p_{z}$. Therefore, we would like the training process of GAN to converge to a good estimator of $p_{data}$ if given enough capacity and training time.　For more details of generative adversarial networks, please refer to \cite{goodfellow2014generative}\cite{arjovsky2017wasserstein}.
...
\multicolumn{1}{l}{\textbf{Dup}} & 0.704 & 0.000 & 0.000 & 0.000 & 0.000 & 0.952 & 1.000 & 0.976 & 0.976 \\ \hline
