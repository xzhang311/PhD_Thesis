...
\Subsubsection{LSTM} Long Short Term Memory networks (LSTMs) are a special kind of recurrent neural network, capable of learning long-term dependencies. LSTMs are explicitly designed to avoid the long-term dependency problem. Remembering information for long periods of time is practically their default behavir. 

LSTMs have the form of a chain of repeating modules of neural network as shown in Figure\ref{fig: lstm}
...
A normal LSTM block contains three gates (input, forget and output), block input, a single cell, an output activation function, and peehole connections. The output of the block is recurrently connected back to the block input and all of the gates.
...
Given a $k$ words sentence $s$, I first map each word to a 400 dimension feature $w_i, 0\leq i \leq k$ called Word2vec embeddings \cite{mikolov2013efficient} which provides a uniform representation of works in feature space and can better encode sementic meaning of words. Then to enable information passing between words and thus learn sementic meaning of an input sentence, after two fully-connected layers, the tranformed work embeddings are enterred to a LSTM unit. 
...
\Subsection{Synthesize Data for Roof Recognition} One example that needs extensive labeling works is object recognition through point cloud. To learn semantic spatial meaning of each point in data requires millions labelling for each single object composed of 3D points, which is impossible to be finished mannually.
...
Although the existing approaches perform well on most cases. It is known that neural network based optical flow estimation can not reliably estimate small motions \cite{Ilg_2017_CVPR}, especially when different magnitude of motions are detected in a same image. The problem actually is understandable from point of view of neural network training process. This is because network neurons are particularly trained to be sensitive to certain patterns detected in images, which means activations/output values of neurons in neural networks are normal distributions centerred at some specific values. Thus when there are multiple type of motions with different magnitude of flows existing in an image, it is very challenging for neural network to encode all kinds of flows in a single flow prediction.
...
Examples of generated intermediately masks are show in Figure \ref{fig: optical flow masks}. As illustrated in the figure, objects with different motion are separated by masks. Such separation will improve the optical flow estimation for corresponding optical flow images of each mask, because by separating object with different motions, corresponding optical flow image only need to focus their prediction on certain type of optical flows, which will producing a more accurate flow estimation. Results of generated optical flows are show in Figure \ref{fig: optical flow comparison}. The generated optical flows are compared to results from DSTFlow \cite{ren2017unsupervised}. It could be seen from the figure that, the proposed approach not only generated flows that much closer to the ground truth results, due to using of mask module in this work, the boundaries of flow between different objects are much more smooth than DSTFlow.
...
Supervised and unsupervised learning of optical flow although have different advantages and as well as disadvantages. They never conflict with each other in this optical flow prediction projects. When ground truth labeling is available, supervised method provide a more precise way of predicting the optical flow. Unsupervise method work as a back up plan when ground truth labeling is not available. In order to increase the accuracy of optical flow estimation to a better level, I condcut another experiment. In this experiment, I combined datasets with know optical flow labels and optical flow predicted using the proposed approach. The combined dataset will be trained supervisely and tested on benchmark dataset.　The idea behind this experiment is that I am going to show when multiple datasets are avaiable but only a small portion of them has ground truth labelings. By training data with true labelings and predicted labelings, we will obtain a better optical flow prediction.
...
KITTI datasets are used as testing set in all of these experiments. 194 KITTI image pairs with ground truth optical flow labels are used as training data in experiment 1 and 3. In experiment 2, KITII training data is used to fine tune the network initially trained using Chairs dataset. There are 82,958 raw image paris in KITTI dataset. 40.000 random picks of them are used to unsupervisely train the proposed network structure. The trained network will then make predictions for remaining 42,958 image pairs of KITTI. We then use predicted optical flows of 42,958 image pairs as  training data in experiment 5, 6 and 7. I use network structure of FlowNet for supervised training for all the supervised experiments. To fine tune pretrained networks, in experiment 2 and 3, 5 epochs using KITTI label data are used to in fine tune.

Experiments resutls are shown in Figure \ref{tab: optical flow experiments train using predicted flows}. From the figure, result of experiment 4 overperforms results of experiment 1, 2 and 3. In experiment 1, 2 and 3, ground truth optical flow from only 194 KITTI image pairs are supervisely trained. Even mixed with Chairs data, the result is still overwhelmed by result from experiment 4 whose network is unsupervisely trained using the proposed approach. This comparison again validate the effectiveness of the propoaed approach. Much better results are obtained in experiments 5, 6 and 7. All three experiments used predicted KITTI optical flow in their training phase. This shows two things: first, predicted optical flow using the proposed approach is of high quality and precision which is also valdiate by experiment 4, second, when mixed predicted KITTI flows with ground truth optical flow from Chair dataset, the training process can learn a better generalization from two different dataset so as to gain an improvement. 
